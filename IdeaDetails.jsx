...
import { 
  ...
  PanelLeftOpen, PanelLeftClose, ChevronLeft, ChevronRight, ChevronDown 
} from 'lucide-react';
import { 
  formatDateToISO, parseSafeDate, getDaysDifference, 
  getEffectiveDates, COLOR_OPTIONS 
} from './utils.js';
import { IconButton } from './components.jsx';

export const ReportsView = ({ ideas, settings, langView, workspaces, activeWorkspaceId, onUpdateIdea, onOpenIdea }) => {
  const [subView, setSubView] = useState('calendar');
  const [selectedWSIds, setSelectedWSIds] = useState(new Set(activeWorkspaceId ? [activeWorkspaceId] : []));
